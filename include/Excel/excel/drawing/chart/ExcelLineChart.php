...
class ExcelLineChart extends ExcelChartCommon
{
    /**
    ...
    public function createDataTable($sheetname) {
        ...
        foreach($this->dataObject->getChartRowData() as $index => $row) {
            ...
            $this->dom->getElementsByTagName("lineChart")->item(0)->appendChild($this->dom->importNode($row, true));
        }
    ...
    public function updateChartDataTableSettings() {
        ...
        if ($this->dataObject->getShowChartDataLabels()) {
            $this->dom->getElementsByTagName("lineChart")->item(0)->getElementsByTagName("dLbls")->item(0)->getElementsByTagName("showVal")->item(0)->setAttribute("val", 1);
        }
        ...
        if ($this->dataObject->getShowChartMarker()) {
            if ($sers = $this->dom->getElementsByTagName("ser")) {
                foreach($sers as $ser) {
                    $marker = $ser->getElementsByTagName("marker")->item(0);
