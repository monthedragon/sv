...
function set_empty_to_dom(empty_tag){
    $('.spn-empty').click(function(){
...
function init_search_fx(url){
		...
		$('.div_activator').click(function(){
			...
			$('.div_list_hid').not('.'+div_list_target).hide();
			$('.'+div_list_target).toggle();
		})
		
		hover_out_tr_fx('tr-alert-list');
		
		$('.tr-alert-list').click(function(){
			var crm_id = $(this).attr('crm_id');
			var calldate = $(this).attr('calldate');
			
			window.location = url+'/'+crm_id+'/'+calldate;
		})
}
