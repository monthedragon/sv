...
class Auth_Controller extends CI_Controller {
	...
	function set_header_data($title='',$sub_title=''){
    ...
    //e.g: $arrOfLuCode = array('gender','status','etc');
    function getLookupBatch($arrOfLuCode)
    {
        $data = array();
        foreach($arrOfLuCode as $lu_code){
            $result =  $this->db->select('*')->from('lookup')
                ->where('is_active',1)
                ->where('lu_cat',$lu_code)
                ->order_by('lu_cat,order_by')
                ->get()
                ->result_array();
            foreach($result as $d)
                $data[$lu_code][$d['lu_code']] = $d['lu_desc'];
        }
